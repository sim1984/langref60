...
Согласно позиционному синтаксису, в вызове процедуры должны присутствовать все аргументы, если у них не указано значение по умолчанию. 
Пропускать аргументы со значением по умолчанию в позиционном синтаксисе, можно только если эти аргументы находятся в конце списка.

Вызов может использовать позиционные, именованные или смешанные аргументы.

Если для аргумента функции существует значение по умолчанию, то вместо указания значения этого аргумента можно использовать ключевое слово `DEFAULT`, в этом случае аргумент будет инициализирован значением по умолчанию.
...
В DSQL параметры могут быть только неименованными, то есть с использованием подстановочного символа `?`.
Использолвание RETURNING_VALUES в DSQL запрещено.
...
Вы можете указать ключевое слово `DEFAULT`, если для параметра задано начение по умолчанию.
