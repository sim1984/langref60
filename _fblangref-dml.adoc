...
.Назначение
Выполнение DSQL оператора с объявленными по месту подпрограммами и/или именованными входными параметрами.
(((USING)))
...
[IMPORTANT]
====
Хотя бы одна из частей `<input_parameter_list>` или `<subroutines>` должна присутствовать. Оператор `USING ... DO`, без входных параметров и подпрограмм, является недопустимым.
====
...
Оператор `USING` упрощает этот процесс. Он предоставляет возможность объявлять параметры и подпрограммы, позволяя при этом механизму автоматически определять выходные данные из содержащейся в них команды SQL.
