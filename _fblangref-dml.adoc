...
Оператор `CALL` вызывает хранимую процедуру. Оператор `CALL` аналогичен оператору `EXECUTE PROCEDURE`, 
но позволяет вызывающей стороне получить определенные выходные параметры или не получить ни одного.
