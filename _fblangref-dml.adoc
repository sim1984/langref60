...
.См. также:
<<fblangref-dml-select-fetch-offset>>, <<fblangref-dml-select-rows>>.
...
[[fblangref-dml-select-orderby-syntax]]
.Синтаксис
[listing,subs=+quotes]
----
SELECT ... FROM ...
...
|value-expression
|Выражение любого типа. Целочисленные литералы рассматривается как позиция столбца.
...
Смотри также <<fblangref-dml-using>>.

[[fblangref-dml-using]]
== `USING`

.Назначение
Выполнение DSQL оператора с объявленными подпрограммами, именованными входными параметрами или в автономной транзакции.
(((USING)))
...
<input_parameter_list> ::= <param_decl> = ? [, <param_decl> = ? ... ]
...
<local_declarations> := <local_declaration> [... <local_declaration>]
...
<local_declaration> ::=
  ...
  | <subproc-declaration>
  | <subproc-implementation>
  | <subfunc-declaration>
  | <subfunc-implementation>
  | <agg-subfunc-declaration>
  | <agg-subfunc-implementation>

<sql_command> :==
    <select-stmt>
  | <insert-stmt>
  | <update-stmt>
  | <update-or-insert-stmt>
  | <delete-stmt>
  | <merge-stmt>
  | <call-stmt>
  | <execute-procedure-stmt>
----

[IMPORTANT]
====
Необходимо наличие хотя бы одной из следующих частей: <input_parameter_list>, <local_declarations> или `IN AUTONOMOUS TRANSACTION`. Оператор `USING DO ...` без параметров, без локальных объявлений и без `IN AUTONOMOUS TRANSACTION` является недопустимым.
====

[[fblangref-dml-tbl-using]]
...
|paramname
|Имя входного параметра. Может содержать до 63 символов. Имя параметра должно быть уникальным среди входных параметров, а также локальных переменных.
...
|local_declarations
|Одна или несколько объявленных локальных переменных, процедур или функций.

|sql_command
|DSQL оператор.
|===

Оператор `USING` -- это расширение DSQL, предназначенное для устранения разрыва между стандартными операторами DSQL и мощным, но многословным оператором `EXECUTE BLOCK`.

При адаптации стандартной команды DSQL для использования `EXECUTE BLOCK` (например, для использования подпрограмм или повторного использования одного входного параметра в нескольких местах) разработчику в настоящее время приходится явно объявлять все входные параметры и, что еще более утомительно, все выходные поля.

Оператор `USING` упрощает этот процесс. Он предоставляет возможность объявлять параметры, подпрограммы и переменные, при необходимости выполнять команду `DO` в автономной транзакции, и при этом движок может автоматически определять выходные данные на основе содержащейся в нём команды SQL.

=== Части оператора `USING`

`<input_parameter_list>`:: Строго типизированный список параметров. Их можно связать со значениями, используя заполнитель `?`.

`<local_declarations>`:: Стандартные локальные объявления PSQL (переменные, подпроцедуры и подфункции).

`IN AUTONOMOUS TRANSACTION`:: Команда `DO` выполняется в отдельной автономной транзакции с использованием той же семантики, которая уже поддерживается PSQL.

`<sql_command>`:: Оператор DSQL для выполнения. Поддерживаемые операторы:

* `SELECT`
* `INSERT` (с `RETURNING` или без него)
* `UPDATE` (с `RETURNING` или без него)
* `UPDATE OR INSERT` (с `RETURNING` или без него)
* `DELETE` (с `RETURNING` или без него)
* `MERGE` (с `RETURNING` или без него)
* `CALL`
* `EXECUTE PROCEDURE`

=== Ключевые особенности

Выводимые выходные парамеры:: В отличие от `EXECUTE BLOCK`, вам не нужно явно объявлять предложение `RETURNS (...)`. +
Выходные столбцы автоматически определяются из `<sql_command>` в предложении `DO`.

Прозрачный тип оператора:: API возвращает тип оператора внутреннего `<sql_command>` (например, если внутренняя команда -- `SELECT`, клиент видит оператор `SELECT`).

Повторное использование параметров:: Входные параметры, объявленные в предложении `USING`, могут использоваться несколько раз в скрипте с помощью именованных ссылок (например, `:p1`), при этом требуется только одна привязка от клиентского приложения.

Смешанная привязка параметров:: Вы можете смешивать объявленные параметры (привязанные через `?` в объявлении) и прямые
позиционные параметры (используя `?` внутри команды `DO`).

=== Примеры

.Повторное использование параметров и подпрограммы
[example]
====
В этом примере демонстрируется объявление типизированных параметров, определение локальных функций/процедур и их использование в запросе.
...
. Клиент связывает значения с `p1` и `p2`.
. Клиент связывает третье значение с `?` внутри предложения `DO`.
. Структура результирующего набора определяется на основе оператора `SELECT`.
====

.Простое повторное использование параметров
[example]
====
Без использования `USING` вставка одного и того же значения привязки в несколько столбцов требует отправки данных дважды.

.Стандартный DSQL
[source,sql]
----
insert into generic_table (col_a, col_b) values (?, ?);
-- Клиент должен выполнить привязку: [100, 100]
----
...
-- Клиент должен выполнить привязку: [100]
----
====

.Выполнение оператора USING внутри EXECUTE STATEMENT с именованными аргументами
[example]
====
[source,sql]
----
...
begin
    execute statement
      ('using(i int = ?, s varchar(20) = ?) do select :i, :s from rdb$database')
      (s := '2', i := 1)
    into ri, rs;
    suspend;
end
...
[IMPORTANT]
====
Обратите внимание, именованные аргументы получают свои имена из блока `USING` через API, без дополнительных замен маркера параметра `?` на `:param_name`. При анализе запроса `EXECUTE STATEMENT` не делает замены выражений вида `:param_name` на `?`. То есть SQL запрос с использованием `USING` используется "как есть".

Для тех, кто пишет свои компоненты доступа для Firebird необходимо предусмотреть специальный режим обработки оператора `USING`, чтобы работать с именованными параметрами.
====

.Выполнение оператора USING внутри EXECUTE STATEMENT с позиционными аргументами
[example]
====
[source,sql]
----
...
.Выполнение оператора USING с локальными переменными
[example]
====
[source,sql]
----
USING
  DECLARE START_VALUE INTEGER = 1;
  DECLARE STOP_VALUE INTEGER = 10;
DO
SELECT
  S.N
FROM
  GENERATE_SERIES(:START_VALUE, :STOP_VALUE) AS S(N)
----
====

.Использование USING для выполнения DSQL оператора в автономной транзакции
[example]
====
[source,sql]
----
...
.Использование USING для выполнения запроса с локальной пользовательской агрегатной функцией
[example]
====
[source,sql]
----
USING
  DECLARE AGGREGATE FUNCTION PRODUCT(I BIGINT)
  RETURNS BIGINT
  AS
    DECLARE R BIGINT;
  BEGIN
...
=== Сравнение EXECUTE BLOCK и USING
...
|*Выходные параметры*
|Выводимые
|Явные (в предложении `RETURNS`)
|Выводимые

|*Варианты использования*
|Простые запросы.
|Сложная логика, циклы, отсутствие DSQL оператора из которого можно получить выходные параметры.
|Повторное использование параметров и подпрограмм для стандартных DSQL запросов.
...
Смотри также <<fblangref-dml-execblock>>.
