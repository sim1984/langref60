...
Не все функции позволяют указывать порядок обработки строк в окне. Например <<fblangref-aggfuncs-withingroup>> не могут иметь предложение `ORDER BY` внутри `OVER`. Такие функции также не позволяют задать рамку окна.
