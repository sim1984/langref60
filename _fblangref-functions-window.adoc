...
<window_specification> ::=
   ( [_window_name_] [<window partition>] [<window order>] [<window frame>] )
...
|expr
|Выражение. Может содержать столбец таблицы, литерал, переменную, выражение, скалярную или агрегатную функцию. Оконные функции в качестве выражения не допускаются.
...
|window frame exclusion
|Выражение для исключения строк из рамки окна.
...
Для каждой строки оконная функция обсчитывает только строки, которые попадают в ту же самую секцию, что и текущая строка.
...
Не все функции позволяют указывать порядок обработки строк в окне. Например, функция <<fblangref-aggfuncs-list>> и <<fblangref-aggfuncs-inv-dist>> не могут иметь предложение `ORDER BY` внутри `OVER`. Такие функции также не позволяют задать рамку окна.
...
Все агрегатные функции могут использовать предложение `ORDER BY`, за исключением `LISTAGG()/LIST()` и функций обратного распределения.
...
(((OVER, RANGE))) (((OVER, ROWS))) (((OVER, GROUPS)))
Набор строк внутри секции, которым оперирует оконная функция, называется _рамкой окна_ (кадры окна). Рамка окна определяет, какие строки следует учитывать для текущей строки при оценке оконной функции.

Рамка окна состоит из трёх частей: единица (unit), начальная граница и конечная граница. В качестве единицы могут быть использованы ключевые слова `RANGE`, `ROWS` или `GROUPS`, которые определяют, каким образом будут работать границы окна.
...
* Если рамка окна задаётся с помощью предложения `GROUPS`, то на предложение `ORDER BY` не накладывается ограничений на количество и типы выражений. В этом случае `<expr> PRECEDING`, `<expr> FOLLOWING` и `CURRENT ROW` относятся к группам-партнерам в разделе. Партнерами являются строки с одинаковыми значениями `ORDER BY`.
...
Фраза `CURRENT ROW` указывает, что окно начинается или заканчивается на текущей строке при использовании совместно с предложением `ROWS`, или что окно заканчивается на текущем значении при использовании с предложением `RANGE` или `GROUPS`. `CURRENT ROW` может быть задана и как начальная, и как конечная точка.

Предложение `BETWEEN` используется совместно с ключевым словом `ROWS`, `RANGE` или `GROUPS` для указания нижней (начальной) или верхней (конечной) граничной точки окна. Верхняя граница не может быть меньше нижней границы.
...
* `FIRST_VALUE`, `LAST_VALUE` и `NTH_VALUE` работают на рамке, но `RANGE` и `GROUPS` работают идентично `ROWS`.

Таким образом, предложения `RANGE`, `ROWS` и `GROUPS` позволяют довольно гибко настроить размер плавающего окна.
...
[[fblangref-windowfuncs-framing-groups]]
=== Окна групп

Окна групп задаются в физических единицах, группах значений (к одной группе относят одинаковые значения аргументов в `ORDER BY`). Например, если рамка окна задана выражением `GROUPS 5 PRECEDING`, то окно будет включать в себя до `6` групп: текущую и пять предыдущих (порядок определяется конструкцией `ORDER BY`).

.Использование окон групп
[example]
====
[source,sql]
----
SELECT
    ...
    SUM(salary) OVER(ORDER BY salary) AS s2,
    SUM(salary) OVER(ORDER BY salary
                     GROUPS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS s3,
    SUM(salary) OVER(ORDER BY salary
                     GROUPS BETWEEN CURRENT ROW AND UNBOUNDED FOLLOWING) AS s4,
    SUM(salary) OVER(ORDER BY salary
                     GROUPS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS s5,
    SUM(salary) OVER(ORDER BY salary
                     GROUPS BETWEEN CURRENT ROW AND 1 FOLLOWING) AS s6,
    SUM(salary) OVER(ORDER BY salary
                     GROUPS BETWEEN 1 PRECEDING AND 1 FOLLOWING) AS s7,
    SUM(salary) OVER(ORDER BY salary GROUPS 1 PRECEDING) AS s8
FROM
    employee;
----
...
3    8.00   49.00    8.00    8.00   49.00   49.00   17.00   17.00    8.00
4    9.00   49.00   17.00   17.00   41.00   49.00   29.00   37.00   17.00
1   10.00   49.00   37.00   37.00   32.00   49.00   32.00   41.00   29.00
5   10.00   49.00   37.00   37.00   32.00   49.00   32.00   41.00   29.00
2   12.00   49.00   49.00   49.00   12.00   49.00   12.00   32.00   32.00
----
====

[[fblangref-windowfuncs-framing-exclude]]
=== Исключение строк из рамки окна
(((OVER, EXCLUDE)))

Необязательное предложение `EXCLUDE` является частью рамки окна и удаляет строки из рамки после оценки его границ. Оно может быть указано только вместе с явной рамкой `RANGE`, `ROWS` или `GROUPS`.

* `EXCLUDE NO OTHERS` является значением по умолчанию и сохраняет рамку без изменений.
* `EXCLUDE CURRENT ROW` удаляет из рамки только текущую строку.
* `EXCLUDE GROUP` удаляет текущую строку и все её аналоги. Аналогами являются строки с теми же значениями `ORDER BY`, что и текущая строка. Если `ORDER BY` отсутствует, все строки в разделе являются аналогами.
* `EXCLUDE TIES` удаляет аналоги текущей строки, но сохраняет саму текущую строку. Если `ORDER BY` отсутствует, все остальные строки в разделе удаляются из рамки.

Например, при наличии дублирующихся зарплат следующий запрос суммирует весь упорядоченный раздел, за исключением группы аналогов текущей зарплаты:

.Использование EXCLUDE GROUP в рамке окна
[example]
====
[source,sql]
----
...
[[fblangref-windowfuncs-withingroup]]
== Функции упорядоченного набора (WITHIN GROUP) как оконные функции
(((WITHIN GROUP)))

Функции упорядоченного набора являются агрегатными функциями, результат которых зависит от порядка обработки записей. Такие функции используются совместно с предложением `WITHIN GROUP` для указания порядка обработки строк. Эти функции могут быть использованы в качестве оконных, но с некоторыми ограничениями:

* Спецификация окна не может содержать предложения `ORDER BY` для задания порядка обработки строк в окне;
* Спецификация окна не может содержать выражение для задания рамки окна;
* Однако спецификация окна может содержать выражения для секционирования строк (`PARTITION BY`);
* В качестве оконных допускается использование функций `LISTAGG/LIST` и функций обратного распределения, но не функций гипотетического множества.

.Использование функции LISTAGG в качестве оконной
[example]
====
[source,sql]
----
...
SELECT
  CODE_BREED,
  CODE_DEPARTURE,
  LISTAGG(NAME) WITHIN GROUP(ORDER BY NAME) OVER() AS BREEDS
FROM BREED;
...
SELECT
  CODE_BREED,
  CODE_DEPARTURE,
  LISTAGG(NAME) WITHIN GROUP(ORDER BY NAME) OVER(PARTITION BY CODE_DEPARTURE) AS BREEDS
FROM BREED;
...
SELECT
  CODE_BREED,
  CODE_DEPARTURE,
  LISTAGG(NAME) WITHIN GROUP(ORDER BY NAME) OVER W AS BREEDS
FROM BREED
WINDOW W AS (PARTITION BY CODE_DEPARTURE);
----
====

[#fblangref-windowfuncs-ex-percentile-disc]
.Использование функции PERCENTILE_DISC в качестве оконной
[example]
====
[source,sql]
----
...
[#fblangref-windowfuncs-ex-percentile-cont]
.Использование функции PERCENTILE_CONT в качестве оконной
[example]
====
[source,sql]
----
...
[NOTE]
====
Функция `PERCENT_RANK` вычисляется как `(RANK-1)/(_total_rows_ - 1)`, где _total_rows_ общее количество строк в секции.
====

.Использование `PERCENT_RANK`
[example]
====
[source,sql]
----
...
|expr
|Выражение.
Может содержать столбец таблицы, литерал, переменную, выражение, неагрегатную функцию или UDF. Агрегатные функции в качестве выражения не допускаются.
|===
...
|expr
|Выражение. Может содержать столбец таблицы, литерал, переменную, выражение, неагрегатную функцию или UDF. Агрегатные функции в качестве выражения не допускаются.

|offset
|Количество строк до строки перед текущей строкой, из которой необходимо получить значение. Если значение аргумента не указано, то по умолчанию принимается 1. _offset_ может быть столбцом, вложенным запросом или другим выражением, с помощью которого вычисляется целая положительная величина, или другим типом, который может быть неявно преобразован в `BIGINT`. _offset_ не может быть отрицательным значением или аналитической функцией.

|default
|Значение по умолчанию, которое возвращается, в случае если смещение (_offset_) указывает за пределы секции. По умолчанию равно `NULL`
|===
