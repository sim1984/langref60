...
Рамка окна состоит из трёх частей: единица (unit), начальная граница и конечная граница. В качестве единицы может быть использовано ключевые слова `RANGE`, `ROWS` или `GROUPS`, которые определяют, каким образом будут работать границы окна.
...
* `FIRST_VALUE`, `LAST_VALUE` и `NTH_VALUE` работают на рамке, но `RANGE` и `GROUPS` работает идентично `ROWS`.
...
== Функции упорядоченного набора как оконные функции

Функции упорядоченного набора являются агрегатными функциями, результат которых зависит от порядка обработки записей. Такие функции могут быть использованы в качестве оконных, но с некоторыми ограничениями:

* Спецификация окна не может содержать предложения `ORDER BY` для задания порядка обработки строк в окне;
* Спецификация окна не может содержать выражения для задание рамки окна;
...
[NOTE]
====
Функция `PERNCENT_RANK` вычисляется как `(RANK-1)/(_total_rows_ - 1)`, где _total_rows_ общее количество строк в секции.
====

.Использование `PERNCENT_RANK`
[example]
====
[source,sql]
----
