...
уровня транзакции (для транзакции `SNAPSHOT` или `CONSISTENCY`) +
или уровня запроса (для транзакции `READ COMMITTED READ CONSISTENCY`). +
...
Например, результатом `mod(7.5, 2.5)` будет 2 (вычисляется как `mod(8, 3)`), а не 0.
====
...
|trim specification
|Из какого места необходимо удалить подстроку -- `BOTH` {vbar} `LEADING` {vbar} `TRAILING`. По умолчанию `BOTH`.

|trim characters
|Подстрока, которую надо удалить (неоднократно, если таких вхождений несколько) из входной строки _str_ в её начале и/или конце. По умолчанию является пробелом (`' '`).
|===
...
|16, 32
|1
|Размер (IV) составляет 8 или 12 байт. Для размера 8 _initial_counter_ -- это 64-битное целое число, для размера 12 -- 32-битное.
...
[WARNING]
====
Если определение домена изменяется, то существующие преобразования к домену или его типу могут стать ошибочными. Если такие преобразования происходят в PSQL модулях, то их ошибки могут быть обнаружены. См. <<fblangref-appx-supp-rdb-validblr>>.
====
...
Функция `GEN_UUID` возвращает универсальный уникальный идентификатор UUID в виде 16-байтной строки символов, отвечающий требованиям стандарта RFC-4122. Функция возвращает строку UUID 4-ой или 7-ой версии (по умолчанию 4-ой), где несколько битов зарезервированы, а остальные являются случайными.
...
Функция `UUID_TO_CHAR` конвертирует 16-ти байтный UUID в его 36-ти знаковое ASCII человеко-читаемое представление.
 ип возвращаемого значения `CHAR(36)`.
...
[example]
====
В данном примере предпринимается попытка использовать все имеющиеся данные для составления полного имени. Выбирается поле `NICKNAME` из таблицы `PERSONS`. Если оно имеет значение `NULL`, то берётся значение из поля `FIRSTNAME`.
