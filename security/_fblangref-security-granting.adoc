...
<packaged_table_privilege_item> ::=
  SELECT | INSERT | UPDATE | DELETE

<packaged_table_privilege> ::=
  <packaged_table_privilege_item> [, <packaged_table_privilege_item> ...]
  ON PACKAGE [_schema_name_.] _package_name_
...
Для таблиц, представлений и пакетов в отличие от других объектов метаданных возможно использовании сразу нескольких привилегий.
...
[[fblangref-security-grant-pkg-table-exmpl]]
==== Примеры `GRANT <privilege>` для упакованных временных таблиц
...
.Предоставление привилегий для упакованных временных таблиц
[example]
====
[source,sql]
----
...
Привилегия `EXECUTE` на пакет позволяет выполнять процедуры и функции пакета, но не другие действия.
...
-- Выдача привилегии USAGE на пакет APP_VAR из схемы APP
-- для всех пользователей
GRANT USAGE ON PACKAGE APP.APP_VAR TO PUBLIC;
...
Привилегия `USAGE` (использование) применима к схемам, а также другим объектам метаданных, таких как исключения, последовательности (генераторы) и пакеты.
...
Привилегия `USAGE` на пакет позволяет ссылаться на константы пакета.
