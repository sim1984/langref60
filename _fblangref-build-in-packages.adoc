...
Системные пакеты предоставляют служебные хранимые процедуры, функции и константы. Они располагаются в системной схеме `SYSTEM`.
...
[[fblangref-build-in-package-blob-util-from-begin]]
=== Константа `RDB$BLOB_UTIL.FROM_BEGIN`

Константа `RDB$BLOB_UTIL.FROM_BEGIN` имеет тип `INTEGER` и значение `0`.

Может быть использована для указания режима поиска в функции `RDB$BLOB_UTIL.SEEK`. При установке в параметр `MODE` константы `RDB$BLOB_UTIL.FROM_BEGIN` поиск будет осуществляться с начала `BLOB`.

См. также <<fblangref-build-in-package-blob-util-seek>>.

[[fblangref-build-in-package-blob-util-from-current]]
=== Константа `RDB$BLOB_UTIL.FROM_CURRENT`

Константа `RDB$BLOB_UTIL.FROM_CURRENT` имеет тип `INTEGER` и значение `1`.

Может быть использована для указания режима поиска в функции `RDB$BLOB_UTIL.SEEK`. При установке в параметр `MODE` константы `RDB$BLOB_UTIL.FROM_CURRENT` поиск будет осуществляться с текущей позиции `BLOB`.

См. также <<fblangref-build-in-package-blob-util-seek>>.

[[fblangref-build-in-package-blob-util-from-end]]
=== Константа `RDB$BLOB_UTIL.FROM_END`

Константа `RDB$BLOB_UTIL.FROM_END` имеет тип `INTEGER` и значение `2`.

Может быть использована для указания режима поиска в функции `RDB$BLOB_UTIL.SEEK`. При установке в параметр `MODE` константы `RDB$BLOB_UTIL.FROM_END` поиск будет осуществляться с конца `BLOB`.

См. также <<fblangref-build-in-package-blob-util-seek>>.

[[fblangref-build-in-package-blob-util-new-blob]]
...
[[fblangref-build-in-package-blob-util-open-blob]]
...
[[fblangref-build-in-package-blob-util-is-writable]]
...
[[fblangref-build-in-package-blob-util-seek]]
...
Параметр `MODE` может быть 0 (с начала), 1 (с текущей позиции) или 2 (с конца). Для указания режима поиска можно использовать соответствующие константы пакета `RDB$BLOB_UTIL.FROM_BEGIN`, `RDB$BLOB_UTIL.FROM_CURRENT` и `RDB$BLOB_UTIL.FROM_END`.
...
См. также: <<fblangref-build-in-package-blob-util-from-begin>>, <<fblangref-build-in-package-blob-util-from-current>>, <<fblangref-build-in-package-blob-util-from-end>>.

[[fblangref-build-in-package-blob-util-cancel-blob]]
...
[[fblangref-build-in-package-blob-util-close-handle]]
...
BEGIN
    ...
    CALL RDB$BLOB_UTIL.CLOSE_HANDLE(bHandle);
END
----
====
...
AS
    ...
    DECLARE bHandle INTEGER;
BEGIN
    -- Создание потокового BLOB во временном хранилище.
    B = RDB$BLOB_UTIL.NEW_BLOB(FALSE, TRUE);
    ...
    -- Установка на позицию 5 с начала BLOB.
    -- Режим поиска задан литералом - 0.
    RDB$BLOB_UTIL.SEEK(bHandle, 0, 5);
    S = RDB$BLOB_UTIL.READ_DATA(bHandle, 3);
    SUSPEND;

    -- Установка на позицию 2 с начала BLOB.
    -- Для указания режима поиска используется константа пакета RDB$BLOB_UTIL.FROM_BEGIN.
    -- Использование констант для режима поиска предпочтительно.
    RDB$BLOB_UTIL.SEEK(bHandle, RDB$BLOB_UTIL.FROM_BEGIN, 2);
    S = RDB$BLOB_UTIL.READ_DATA(bHandle, 3);
    SUSPEND;

    -- Перемещение позиции на 2 относительно текущей позиции BLOB.
    RDB$BLOB_UTIL.SEEK(bHandle, RDB$BLOB_UTIL.FROM_CURRENT, 2);
    S = RDB$BLOB_UTIL.READ_DATA(bHandle, 3);
    SUSPEND;

    -- Перемещение позиции на -1 с конца BLOB.
    RDB$BLOB_UTIL.SEEK(bHandle, RDB$BLOB_UTIL.FROM_END, -1);
    S = RDB$BLOB_UTIL.READ_DATA(bHandle, 3);
    SUSPEND;
END!
...
BEGIN
    ...
    INSERT INTO T (B) VALUES ('not writable')
    RETURNING B INTO B;
