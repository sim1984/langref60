...
[NOTE]
====
Табличные функции очень похожи на хранимые процедуры выбора, но в отличие от процедур в настоящее время пользователь не может создавать свои табличные функции. Таким образом, табличную функцию можно рассматривать как встроенную хранимую процедуру выбора.
====

.Неоднозначность при вызове хранимых процедур
[WARNING]
====
Поскольку синтаксис выборки строк из хранимых процедур выбора очень похож на синтаксис выборки из встроенных табличных функций, то между ними может возникнуть неоднозначность, в том случае если имя процедуры и встроенной табличной функции совпадают.

Firebird будет вызывать встроенную табличную функцию, если присутствует предложение корреляции (`[AS] ...`) и количество аргументов соответствует табличной функции, в противном случае будет вызвана одноимённая хранимая процедура из текущего пути поиска.

Если вам необходимо производить выборку из одноимённой хранимой процедуры с указанием предложения корреляции, то необходимо уточнить схему для этой хранимой процедуры, например `PUBLIC.UNLIST` или `PUBLIC.GENERATE_SERIES`.
====
...
[[fblangref-tablefunc-unlist-in]]
=== Сокращённый синтаксис `IN UNLIST`

Табличная функция `UNLIST` может быть использована в предикате `IN` в сокращённой форме.

.Синтаксис
[listing,subs="+quotes, macros"]
----
<expr> [NOT] IN UNLIST (<input> [, <separator>] [<data type conversion>])
...
Особенности использования сокращённого синтаксиса `IN UNLIST`:

* не требуется предложение корреляции `[AS] <correlation name> [(<derived column name>)]`;
* если не указано предложение для приведения типа `RETURNING <data type>`, то по умолчанию будет подставлен тип выражения левого операнда `IN`, то есть `<expr>`.
...
[source,sql]
----
SELECT * FROM EMPLOYEE WHERE EMP_NO IN UNLIST(?, ',');
----
...
WHERE DEPT_NO IN UNLIST('100:110:115:120', ':' RETURNING INT);
----
...
[IMPORTANT]
====
Сокращённый синтаксис `IN UNLIST` является синтаксическим сахаром над полной формой. Таким образом, Firebird всегда будет выполнять оператор `IN` как будто внутри находится подзапрос. Это может быть важно при оптимизации запросов.
====
...
в реальности будет выполнен как

[source,sql]
----
SELECT * FROM EMPLOYEE
WHERE EMP_NO IN (
    SELECT * FROM UNLIST('2,4,5,7,11' RETURNING INT) AS U
);
----

Смотрите также: <<fblangref-commons-in>>.
...
|finish
|Конечное значение интервала. `finish` может быть переменной, литералом или скалярным выражением типа `SMALLINT`, `INTEGER`, `BIGINT`, `INT128` или `NUMERIC/DECIMAL`. Генерация последовательности останавливается, когда последнее сгенерированное значение шага превышает или становится меньше (зависит от знака `step`) значения `finish`.

|step
|Значение, на которое изменяется результат между рядами на каждом шаге выполнения. `step` может быть выражением типа `SMALLINT`, `INTEGER`, `BIGINT`, `INT128` или `NUMERIC/DECIMAL`. Значение `step` может быть как отрицательным, так и положительным, но не может быть равно нулю (0). Этот аргумент необязателен. По умолчанию значение `step` равно 1.
...
[source,sql]
----
SELECT * FROM GENERATE_SERIES(1, 3) AS S;
----
