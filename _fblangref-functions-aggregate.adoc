...
Поле `EMP_NO` является первичным ключом и уже обеспечивает уникальность сотрудников, однако SQL требует, чтобы в `GROUP BY` содержались все выражения из `SELECT` кроме агрегатных функций. Очевидно что запрос с использованием агрегатных функций `ANY_VALUE` менее затратен, чем запрос с группировками по дополнительным столбца. Раньше вместо агрегатной функции `ANY_VALUE` для этих же целей использовались функции `MIN` или `MAX`. Вычисление `ANY_VALUE` более дёшево, поскольку `MIN/MAX` требуют сравнения значений, в то время как `ANY_VALUE` просто возвращает первое попавшееся значение в группе.
