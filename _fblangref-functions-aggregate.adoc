...
Устаревший синтаксис `LIST` сохранен для обратной совместимости и является синонимом для функции `LISTAGG`. В новых проектах рекомендуется использовать функцию `LISTAGG`, поскольку она является совместимой с SQL стандартом.
...
Предложение `DISTINCT` учитывает направление сортировки, если первый аргумент сортировки в `WITHIN GROUP` имеет тоже выражение, что используется для конкатенации.
...
Сортировка в `WITHIN GROUP` может быть разнонаправленная для каждого из аргументов сортировки.
...
* При пустой выборке или при выборке из одних `NULL` функция вернёт `NULL`.
...
|percent
|Значение процентиля. Должно находится в диапазоне между 0 и 1. Это выражение должно оставаться постоянным внутри каждой агрегированной группы.
...
|args
|Аргументы (выражения) задающие гипотетическую строку. Эти выражения должны оставаться постоянным внутри каждой агрегированной группы.
