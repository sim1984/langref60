...
[[fblangref-aggfuncs-withingroup]]
== Функции упорядоченного набора

Функция упорядоченного набора (ordered set function) -- это особый вид агрегирующей функции. От обычных функций (таких как `SUM`, `MIN`, `МАХ` и прочих) их отличает значимость порядка при вычислениях, например порядок элементов при конкатенации.

Функции упорядоченного набора используются в групповых запросах аналогично обычным функциям набора. Что касается синтаксиса, в стандарте SQL определено предложение `WITHIN GROUP`, в котором можно задать правило упорядочивания. Общий синтаксис функций упорядоченного набора выглядит следующим образом:

[listing, subs="+quotes, macros"]
----
<ordered_set_function> ::=
  _agg_func_ ( [ [ALL | DISTINCT] <args>] )
  WITHIN GROUP ( ORDER BY <sort_expr> )
  [FILTER (WHERE <condition>)]
----

Функции упорядоченного набора также могут использоваться в качестве оконных, подробнее см. <<fblangref-windowfuncs-withingroup>>.

К функциям упорядоченного набора относятся следующие:

* `LISTAGG` или `LIST`;
* Функции обратного распределения `PERCENTILE_CONT` и `PERCENTILE_DISC`;
* Функции гипотетического набора: `RANK`, `DENSE_RANK`, `PERCENT_RANK` и `CUME_DIST`.
...
В следующий примерах показано как учитываются значения `NULL` в сортировке.
...
Функция `PERCENTILE_DISC` -- это обратная функция распределения, предполагающая дискретную модель распределения. Она принимает значение процентиля и спецификацию сортировки и возвращает элемент из множества. Нулевые значения игнорируются при вычислении.
...
Предложение `ORDER BY` принимает одно выражение `<order_expr>` любого типа, которое может быть отсортировано. Например, выражение возражающее `BLOB` не допускается. Это выражение также считается вторым неявным аргументом функции `PERCENTILE_DISC`.
...
Функции гипотетических множеств включают функции ранжирования и распределения рангов, с которые существуют также в виде оконных функций (см. <<fblangref-windowfuncs-rankfuncs>>), но они применяются к группам для ввода значений в гипотетической форме.
