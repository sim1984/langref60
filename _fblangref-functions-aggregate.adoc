...
----
<aggregate_function> ::=
  _aggregate_function_ ([ALL | DISTINCT] <expr>)
...
Выделяют следующие группы агрегатных функций:

* <<fblangref-aggfuncs-general,Основные агрегатные функции>>
* <<fblangref-aggfuncs-bitwise,Побитовые агрегатные функции>>
* <<fblangref-aggfuncs-inv-dist,Функции обратного распределения>>
* <<fblangref-aggfuncs-hypotset,Функции гипотетического набора>>
* <<fblangref-aggfuncs-stats,Статистические функции>>
* <<fblangref-aggfuncs-regr,Функции линейной регрессии>>
* <<fblangref-psql-function-agg,Пользовательские агрегатные функции>>
...
[[fblangref-aggfuncs-within-group]]
== Предложение `WITHIN GROUP` для агрегатных функций
(((WITHIN GROUP)))

Предложение `WITHIN GROUP` используется для некоторых видов агрегатных функций, чтобы определить порядок их выполнения или обработки внутри группы. Такие функции отличает значимость порядка при вычислениях, например порядок элементов при конкатенации.
...
<sort-specification-list> ::=
  !! См. синтаксис <<fblangref-dml-select-orderby-syntax, ORDER BY>> !!
----

Предложение `WITHIN GROUP` поддерживают следующие функции:

* <<fblangref-aggfuncs-list>>;
* <<fblangref-aggfuncs-inv-dist>>;
* <<fblangref-aggfuncs-hypotset>>.

Для функций обратного распределения спецификация сортировки в предложении `WITHIN GROUP` может содержать только один элемент.
...
|expr
|Выражение. Может содержать столбец таблицы, литерал, переменную, выражение, неагрегатную функцию или UDF, которая возвращает числовой тип данных.
|===
...
Значения `NULL` игнорируются. Значение `NULL` возвращается только в случае отсутствия записей, значение выражения которых отличается от `NULL`.
...
* Для пустой выборки данных или если выборка содержит только значения `NULL`, функция возвратит значение равное `0`.
...
.Доступно в
DSQL
(((Функция, `LIST()`)))(((Функция, `LISTAGG()`)))

.Синтаксис
[listing,subs=+quotes]
----
<list_agg_function> ::=
  {LIST | LISTAGG} ([ALL | DISTINCT] <expr> [, _separator_])
...
.Параметры функции LIST / LISTAGG
[cols="<1,<3", options="header",stripes="none"]
|===
^| Параметр
^| Описание
...
|expr
|Выражение. Может содержать столбец таблицы, литерал, переменную, выражение, неагрегатную функцию или UDF, которая возвращает строковый тип данных или `BLOB`. Поля типа дата, время и числовые преобразуются к строке.

|separator
|Разделитель. Выражение строкового типа. По умолчанию разделителем является запятая.

|listagg truncation filler
|Заполнитель при переполнении. Строковый литерал.

|sort specification list
|Выражение сортировки, определяющее порядок конкатенации.
|===

.Тип возвращаемого результата
`BLOB`

Агрегатная функция `LISTAGG` предназначена для конкатенации значений аргумента. Она возвращает строку, состоящую из значений аргумента, отличных от `NULL` в группе, разделенных запятой или заданным пользователем разделителем. Если нет значений, отличных от `NULL` (включая случай, когда группа пуста), возвращается `NULL`.

Устаревший синтаксис `LIST` сохранён для обратной совместимости и является синонимом для функции `LISTAGG`. В новых проектах рекомендуется использовать функцию `LISTAGG`, поскольку она является совместимой с SQL стандартом.

Если перед выражением _expr_ указано ключевое слово  `ALL` (по умолчанию), то это приводит к обработке всех значений, отличных от `NULL`. Если указано ключевое слово `DISTINCT`, то дубликаты удаляются, за исключением случаев, когда _expr_ является `BLOB`.

Необязательный аргумент _separator_ может быть любым строковым выражением. Это позволяет указать, например, `ascii_char (13)` в качестве разделителя.

Аргументы _expr_ и _separator_ поддерживают `BLOB` любого размера и набора символов. Дата / время и числовые аргументы неявно преобразуются в строки перед объединением. Результатом функции `LISTAGG` является текстовый `BLOB`, кроме случаев, когда _expr_ является `BLOB` другого подтипа.

В SQL стандарте есть правило `<listagg overflow clause>`, которое предназначено для обработки ситуации, возникающей при переполнении выходного результата. Поскольку функция `LISTAGG` всегда возвращает BLOB-объект, то это правило бессмысленно. Оно не реализовано и если указано, то молча игнорируется.

Порядок значений в списке определяется выражением сортировки, указанным в опции `WITHIN GROUP`. Если спецификация `WITHIN GROUP` отсутствует, то порядок конкатенации не определён. Если указано ключевое слово `DISTINCT`, то сортировка, указанная в `WITHIN GROUP`, игнорируется, за исключением случая, когда `WITHIN GROUP (ORDER BY <sort specification list>)` упоминает агрегированное выражение в самом начале своего списка `<sort specification list>` -- в таких случаях направление сортировки учитывается внутри `DISTINCT`.

[[fblangref-aggfuncs-list-exmpl]]
==== Примеры `LIST / LISTAGG`

.Использование функции LISTAGG
[example]
====
...
SELECT LISTAGG (display_name, '; ')
FROM GR_WORK;
----
====

.Использование функции LISTAGG с заданным порядком
[example]
====
...
SELECT LIST (display_name, '; ') WITHIN GROUP (ORDER BY display_name)
FROM GR_WORK;
----
====

.Использование функции LISTAGG с заданным порядком и группировкой
[example]
====
Получение списка в порядке, обратном алфавитному, и сгруппированному по столбцу `work_type`.
...
.Использование функции LISTAGG с заданным порядком и исключением дубликатов
[example]
====
Получение списка в порядке, обратном алфавитному, и сгруппированному по столбцу `work_type`. Дубликаты имён удаляются.

[source,sql]
----
SELECT
  work_type,
  LISTAGG (DISTINCT display_name, '; ') WITHIN GROUP (ORDER BY display_name DESC) as works
FROM GR_WORK
GROUP BY work_type;
----
====

Далее приведены примеры различных вариантов использования функции `LISTAGG` и их результаты. В любом из этих примеров вы можете заменить `LISTAGG` на `LIST` без изменения результата.
...
В примере ниже дубликаты значений столбца `COL3` удаляются. Направление сортировки, указанное в `WITHIN GROUP`, учитывается, если выражение сортировки в `WITHIN GROUP` совпадает с аргументом конкатенации.
...
----
SELECT
  LISTAGG (DISTINCT COL3, ':') WITHIN GROUP (ORDER BY COL3 ASCENDING)
FROM TEST_T;
----
...
----
SELECT
  LISTAGG (DISTINCT COL3, ':') WITHIN GROUP (ORDER BY COL3 DESCENDING)
FROM TEST_T;
----
...
Предложение `DISTINCT` учитывает направление сортировки, если первый аргумент сортировки в `WITHIN GROUP` имеет то же выражение, которое используется для конкатенации.

[source,sql]
----
SELECT
  LISTAGG (DISTINCT COL3, ':') WITHIN GROUP (ORDER BY COL3 DESCENDING, COL4, COL5)
FROM TEST_T;
----
...
Если выражение первого аргумента сортировки в `WITHIN GROUP` отличается от того, что используется для конкатенации, то сортировка игнорируется.

[source,sql]
----
SELECT
  LISTAGG (DISTINCT COL3, ':') WITHIN GROUP (ORDER BY COL4, COL3 DESCENDING, COL5)
FROM TEST_T;
----
...
Вышесказанное справедливо, если аргумент сортировки только один.

[source,sql]
----
SELECT
  LISTAGG (DISTINCT COL3, ':') WITHIN GROUP (ORDER BY COL2)
FROM TEST_T;
----
...
----
SELECT
  LISTAGG (DISTINCT COL3, ':') WITHIN GROUP (ORDER BY COL2 DESCENDING)
FROM TEST_T;
----
...
Сортировка в `WITHIN GROUP` может быть разнонаправленной для каждого из аргументов сортировки.

[source,sql]
----
SELECT
  LISTAGG (COL2, ':') WITHIN GROUP (ORDER BY COL3 DESC, COL4 ASC)
FROM TEST_T;
----
...
----
SELECT
  LISTAGG (COL2, ':') WITHIN GROUP (ORDER BY COL3 DESC, COL4 DESC)
FROM TEST_T;
----
...
----
SELECT
  LISTAGG (COL2, ':') WITHIN GROUP (ORDER BY COL3 ASC, COL4 DESC)
FROM TEST_T;
----
...
Предложение `OVERFLOW ...` игнорируется и не влияет на результат.

[source,sql]
----
SELECT
  LISTAGG (COL4, ':' ON OVERFLOW TRUNCATE '...' WITHOUT COUNT) WITHIN GROUP (ORDER BY COL3 ASC)
FROM TEST_T;
----
...
----
SELECT
  LISTAGG (COL4, ':' ON OVERFLOW TRUNCATE '...' WITH COUNT) WITHIN GROUP (ORDER BY COL3 DESC)
FROM TEST_T;
----
...
----
SELECT
  LISTAGG (DISTINCT COL3, ':' ON OVERFLOW ERROR) WITHIN GROUP (ORDER BY COL3)
FROM TEST_T;
----
...
В следующих примерах показано, как учитываются значения `NULL` в сортировке.
...
SELECT LISTAGG (ALL COL2, ':') WITHIN GROUP (ORDER BY COL3)
FROM TEST_T;
----
...
----
SELECT
  LISTAGG (ALL COL2, ':') WITHIN GROUP (ORDER BY COL3 NULLS LAST)
FROM TEST_T;
----
...
----
SELECT
  LISTAGG (ALL COL2, ':') WITHIN GROUP (ORDER BY COL6 NULLS FIRST)
FROM TEST_T;
----
...
Функция `MAX` возвращает максимальный элемент выборки, который не равен `NULL`.
...
[NOTE]
====
Параметр `DISTINCT` не имеет смысла при использовании функции `MAX` и доступен только для совместимости со стандартом.
====
...
Функция `MIN` возвращает минимальный элемент выборки, который не равен `NULL`.
...
[NOTE]
====
Параметр `DISTINCT` не имеет смысла при использовании функции `MIN` и доступен только для совместимости со стандартом.
====
...
* При пустой выборке или при выборке, содержащей только `NULL`, функция вернёт `NULL`.
...
Функция `BIN_AND_AGG` выполняет побитовую операцию `AND` для всех значений аргумента в группе. Значения `NULL` пропускаются.
...
Функция `BIN_OR_AGG` выполняет побитовую операцию `OR` для всех значений аргумента в группе. Значения `NULL` пропускаются.
...
Функция `BIN_XOR_AGG` выполняет побитовую операцию `XOR` для всех значений аргумента в группе. Значения `NULL` пропускаются.
...
.См. также:
<<fblangref-scalarfuncs-bin-xor>>, <<fblangref-aggfuncs-bin-or-agg>>, <<fblangref-aggfuncs-bin-and-agg>>.

[[fblangref-aggfuncs-inv-dist]]
== Функции обратного распределения

Функции обратного распределения принимают значение процентиля и спецификацию сортировки и возвращают элемент из множества. Эти функции также могут использоваться в качестве оконных, подробнее см. <<fblangref-windowfuncs-withingroup>>.

Функции обратного распределения используются в групповых запросах аналогично обычным функциям набора. Что касается синтаксиса, в стандарте SQL определено предложение `WITHIN GROUP`, в котором можно задать правило упорядочивания.

[[fblangref-aggfuncs-percentile-disc]]
=== `PERCENTILE_DISC()`
...
----
<percentile-disc-agg> ::=
  PERCENTILE_DISC(<percent>) WITHIN GROUP (ORDER BY <order_expr> [ASC | DESC])
...
[[fblangref-aggfuncs-tbl-percentile-disc]]
.Параметры функции PERCENTILE_DISC
[cols="<1,<3", options="header",stripes="none"]
|===
^| Параметр
^| Описание
...
|percent
|Значение процентиля. Должно находиться в диапазоне между 0 и 1. Это выражение должно оставаться постоянным внутри каждой агрегированной группы.

|order_expr
|Выражение сортировки. Может включать только *один* элемент сортировки.

|part_expr
|Выражение секционирования окна.

|===

.Тип возвращаемого результата
тот же, что и тип выражения `<order_expr>`.

Функция `PERCENTILE_DISC` -- это обратная функция распределения, предполагающая дискретную модель распределения. Она принимает значение процентиля и спецификацию сортировки и возвращает элемент из множества. Значения `NULL` игнорируются при вычислении.

Первый аргумент `<percent>` должен принимать числовое значение от 0 до 1, поскольку это значение процентиля. Это выражение должно быть постоянным в каждой группе агрегатов.

Предложение `ORDER BY` принимает одно выражение `<order_expr>` любого типа, которое может быть отсортировано. Например, выражение, возвращающее `BLOB`, не допускается. Это выражение также считается вторым неявным аргументом функции `PERCENTILE_DISC`.

Функция `PERCENTILE_DISC` возвращает значение того же типа, что и аргумент в предложении `ORDER BY`.

Для заданного значения процентиля `P` функция `PERCENTILE_DISC` сортирует значения выражения в предложении `ORDER BY` и возвращает значение с наименьшим значением `CUME_DIST` (относительно той же спецификации сортировки), которое больше или равно `P`.

.Использование функции агрегатной `PERCENTILE_DISC` для получения медианы
[example]
====
[source,sql]
----
...
Пример использования `PERCENTILE_DISC` в качестве оконной функции см. <<fblangref-windowfuncs-ex-percentile-disc>>.

[[fblangref-aggfuncs-percentile-cont]]
=== `PERCENTILE_CONT()`
...
----
<percentile-disc-agg> ::=
  PERCENTILE_CONT(<percent>) WITHIN GROUP (ORDER BY <order_expr> [ASC | DESC])
...
[[fblangref-aggfuncs-tbl-percentile-cont]]
.Параметры функции PERCENTILE_CONT
[cols="<1,<3", options="header",stripes="none"]
|===
^| Параметр
^| Описание
...
.Тип возвращаемого результата
`DOUBLE PRECISION` или `DECFLOAT(34)`.

`PERCENTILE_CONT` -- это функция обратного распределения, предполагающая модель непрерывного распределения. Она принимает значение процентиля и спецификацию сортировки и возвращает элемент из множества. Значения `NULL` игнорируются при вычислении.

Первый аргумент `<percent>` должен принимать числовое значение от 0 до 1, поскольку это значение процентиля. Это выражение должно оставаться постоянным внутри каждой агрегированной группы.

Предложение `ORDER BY` принимает одно выражение, которое должно быть числового типа для выполнения интерполяции. Это выражение также считается вторым неявным аргументом функции `PERCENTILE_CONT`.

Функция `PERCENTILE_CONT` возвращает значение типа `DOUBLE PRECISION` или `DECFLOAT(34)` в зависимости от типа
аргумента в предложении `ORDER BY`. Значение типа `DECFLOAT(34)` будет возвращено, если `ORDER BY` содержит
выражение одного из типов `INT128`, `NUMERIC(38, _x_)` или `DECFLOAT(16 | 34)`, в противном случае возвращаемое значение будет типа `DOUBLE PRECISION`.

Результат функции `PERCENTILE_CONT` вычисляется путем линейной интерполяции между значениями после их упорядочивания. Используя значение процентиля (`P`) и количество строк (`N`) в группе агрегирования, можно вычислить номер интересующей строки после упорядочивания строк в соответствии со спецификацией сортировки. +
Этот номер строки (`RN`) вычисляется по формуле `RN = 1 + (P * (N - 1))`. +
Конечный результат функции агрегирования вычисляется путем линейной интерполяции между значениями из строк с номерами строк `CRN = CEILING(RN)` и `FRN = FLOOR(RN)`.
...
.Использование функции агрегатной `PERCENTILE_CONT` для получения медианы
[example]
[example]
====
[source,sql]
...
Пример использования `PERCENTILE_CONT` в качестве оконной функции см. <<fblangref-windowfuncs-ex-percentile-cont>>.

[[fblangref-aggfuncs-hypotset]]
== Функции гипотетического набора

Функции гипотетических множеств включают функции ранжирования и распределения рангов, которые существуют также в виде оконных функций (см. <<fblangref-windowfuncs-rankfuncs>>), но они применяются к группам для ввода значений в гипотетической форме.

Эти функции отвечают на вопрос: какой был бы ранг строки, заданный в аргументах функции, если бы он присутствовал в сортированном наборе строк.

Существуют две функции ранжирования отсортированных множеств: `RANK` и `DENSE_RANK`. Также существуют две функции распределения рангов отсортированных множеств: `PERCENT_RANK` и `CUME_DIST`. Оконные функции и функции упорядоченных множеств различаются способом упорядочивания. Первые упорядочивают внутри окна, а вторые -- внутри группы.

.Синтаксис агрегатных функций гипотетического множества
[listing,subs=+quotes]
----
...
Каждый из агрегатов "`гипотетического множества`" связан с оконной функцией с тем же именем. В каждом случае результатом агрегата является значение, которое соответствующая оконная функция вернула бы для "`гипотетической`" строки, если бы такая строка была добавлена к отсортированной группе строк, представленной `sorted_args`. Для каждой из этих функций список прямых аргументов, заданных в `args`, должен соответствовать количеству и типам агрегируемых аргументов, заданных в `sorted_args`. В отличие от большинства встроенных агрегатов, эти агрегаты не являются строгими, то есть они не удаляют входные строки, содержащие значения `NULL`. Значения `NULL` сортируются в соответствии с правилом, указанным в предложении `ORDER BY`.

Значения входных аргументов `args` должны быть постоянными внутри каждой группы.

[[fblangref-aggfuncs-hypotset-dense-rank]]
=== `DENSE_RANK()`

.Доступно в
DSQL
(((Агрегатная функция, `DENSE_RANK()`)))
...
[[fblangref-aggfuncs-tbl-dense-rank]]
.Параметры функции DENSE_RANK
[cols="<1,<3", options="header",stripes="none"]
|===
^| Параметр
^| Описание
...
|args
|Аргументы (выражения) задающие гипотетическую строку. Эти выражения должны оставаться постоянными внутри каждой агрегированной группы.

|sorted_args
|Аргументы (выражения) сортировки, определяющие группу сортированных строк, в которую включается гипотетическая строка. Количество аргументов сортировки должно совпадать с количеством аргументов, задающих гипотетическую строку. Для каждого элемента сортировки может быть задано направление `ASC\|DESC` и положение значения `NULL` (`NULLS {FIRST \| LAST}`).
...
Функция `DENSE_RANK()` возвращает ранг каждой гипотетической строки в пределах группы, заданной набором входных аргументов `<args>`. Строки с одинаковыми значениями `<sorted_args>` получают одинаковый ранг в пределах группы. Ранг строки равен количеству различных значений рангов в группе, предшествующих текущей строке, увеличенному на единицу.

.Использование функции `DENSE_RANK()` для получения ранга гипотетической строки
[example]
====
[source,sql]
----
SELECT
  DENSE_RANK(35500) WITHIN GROUP(ORDER BY E.SALARY) AS R
FROM EMPLOYEE E;
----
...
.Использование функции `DENSE_RANK()` для получения ранга гипотетической строки в группе
[example]
====
[source,sql]
----
...
.Использование функции гипотетического набора `DENSE_RANK()` с двумя аргументами
[example]
====
[source,sql]
----
SELECT
  DENSE_RANK('England', 35500) WITHIN GROUP(ORDER BY E.JOB_COUNTRY, E.SALARY) AS R
FROM EMPLOYEE E;
----
...
См. также <<fblangref-windowfuncs-dense-rank,Оконная функция DENSE_RANK()>>.

[[fblangref-aggfuncs-hypotset-rank]]
=== `RANK()`

.Доступно в
DSQL
(((Агрегатная функция, `RANK()`)))
...
Функция `RANK()` возвращает ранг гипотетической строки в пределах группы, заданной набором входных аргументов `<args>`. Строки с одинаковыми значениями `<sorted_args>` получают одинаковый ранг в пределах группы. Ранг строки вычисляется как единица плюс количество рангов, находящихся до этой строки.

.Использование функции `RANK()` для получения ранга гипотетической строки
[example]
====
[source,sql]
----
SELECT
  RANK(35500) WITHIN GROUP(ORDER BY E.SALARY) AS R
FROM EMPLOYEE E;
----
...
.Использование функции `RANK()` для получения ранга гипотетической строки в группе
[example]
====
[source,sql]
----
...
.Использование функции гипотетического набора `RANK()` с двумя аргументами
[example]
====
[source,sql]
----
SELECT
  RANK('England', 35500) WITHIN GROUP(ORDER BY E.JOB_COUNTRY, E.SALARY) AS R
FROM EMPLOYEE E;
----
...
См. также <<fblangref-windowfuncs-rank,Оконная функция RANK()>>.

[[fblangref-aggfuncs-hypotset-percent-rank]]
=== `PERCENT_RANK()`

.Доступно в
DSQL
(((Агрегатная функция, `PERCENT_RANK()`)))
...
[[fblangref-aggfuncs-tbl-percent-rank]]
.Параметры функции PERCENT_RANK
[cols="<1,<3", options="header",stripes="none"]
|===
^| Параметр
^| Описание
...
Возвращает относительный ранг гипотетической строки в группе строк. Функция `PERCENT_RANK` используется для вычисления относительного положения значения в группе или результирующем наборе запроса. Диапазон значений, возвращаемый функцией `PERCENT_RANK`, больше 0 и меньше или равен 1.

[NOTE]
====
Функция `PERCENT_RANK` вычисляется как `(RANK-1)/(_total_rows_ - 1)`, где _total_rows_ — общее количество строк в группе с учётом гипотетической строки.
====

.Использование функции `PERCENT_RANK()` для получения относительного ранга гипотетической строки
[example]
====
[source,sql]
----
...
.Использование функции `PERCENT_RANK()` для получения относительного ранга гипотетической строки в группе
[example]
====
[source,sql]
----
...
JOB_COUNTRY                          PR
=============== =======================
Canada                0.000000000000000
England              0.6666666666666666
France                0.000000000000000
Italy                 1.000000000000000
Japan                 0.000000000000000
Switzerland           0.000000000000000
USA                  0.1515151515151515
----
====

.Использование функции гипотетического набора `PERCENT_RANK()` с двумя аргументами
[example]
====
[source,sql]
----
...
См. также <<fblangref-windowfuncs-percent-rank,Оконная функция PERCENT_RANK()>>.

[[fblangref-aggfuncs-hypotset-cume-dist]]
=== `CUME_DIST()`

.Доступно в
DSQL
(((Агрегатная функция, `CUME_DIST()`)))
...
[[fblangref-aggfuncs-tbl-cume-dist]]
.Параметры функции CUME_DIST
[cols="<1,<3", options="header",stripes="none"]
|===
^| Параметр
^| Описание
...
Функция `CUME_DIST` рассчитывает кумулятивное распределение значения в наборе данных для гипотетической строки. Возвращаемое значение находится в диапазоне от 0 до 1. Функция `CUME_DIST` рассчитывается как (число строк, предшествующих или равных гипотетической) / (общее число строк с учётом гипотетической). Для равных значений всегда вычисляется одно и то же значение накопительного распределения.

.Использование функции `CUME_DIST()` для получения кумулятивного распределения гипотетической строки
[example]
====
[source,sql]
----
...
.Использование функции `CUME_DIST()` для получения кумулятивного распределения гипотетической строки в группе
[example]
====
[source,sql]
----
...
JOB_COUNTRY                          CD
=============== =======================
Canada               0.5000000000000000
England              0.7500000000000000
France               0.5000000000000000
Italy                1.0000000000000000
Japan                0.3333333333333333
Switzerland          0.5000000000000000
USA                  0.1764705882352941
----
====

.Использование функции гипотетического набора `CUME_DIST()` с двумя аргументами
[example]
====
[source,sql]
----
SELECT
  CUME_DIST('England', 35500) WITHIN GROUP(ORDER BY E.JOB_COUNTRY, E.SALARY) AS R
FROM EMPLOYEE E;
----
...
|expr1, expr2
|Выражение возвращает числовой тип данных. Может содержать столбец таблицы, литерал, переменную, выражение, неагрегатную функцию или UDF.
Агрегатные функции в качестве выражения не допускаются.
|===
...
Может содержать столбец таблицы, литерал, переменную, выражение, неагрегатную функцию или UDF, которая возвращает числовой тип данных. Агрегатные функции в качестве выражения не допускаются.
