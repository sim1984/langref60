...
Порядок значений в списке определяется выражением сортировки, указанном в опции `WITHIN GROUP`. Если спецификация `WITHIN GROUP` отсутствует, то порядок конкатенации не определён.
