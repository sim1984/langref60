...
В стандарте есть правило `<listagg overflow clause>`, которое предназначено для вывода ошибки при переполнении выходного результата. Поскольку функция `LISTAGG` всегда возвращает BLOB-объект, то это правило бессмысленно. Оно не  реализовано и, если указано, то  молча игнорируется.
...
// TODO: взаимодействие DISTINCT с WITHIN GROUP
// TODO: примеры
