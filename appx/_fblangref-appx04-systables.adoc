...
[[fblangref-systables-constants]]
== `RDB$CONSTANTS`

Содержит информацию о константах пакетов.

.Описание столбцов таблицы `RDB$CONSTANTS`
[cols="<4m,<3m,<5", frame="all", options="header",stripes="none"]
|===
^| Наименование столбца
^| Тип данных
...
|RDB$PACKAGE_NAME
|CHAR(63) CHARACTER SET UTF8
|Имя пакета в котором объявлена константа.

|RDB$FIELD_SOURCE
|CHAR(63) CHARACTER SET UTF8
|Имя домена, созданного пользователем (при использовании ссылки на домен вместо типа), или домена, автоматически построенного системой для константы. Во втором случае имя будет начинаться с символов `RDB$`.

|RDB$FIELD_SOURCE_SCHEMA_NAME
|CHAR(63) CHARACTER SET UTF8
|Имя схемы домена для константы.

|RDB$PRIVATE_FLAG
|SMALLINT
|Признак доступности константы вне пакета, где она объявлена.

0 -- если константа описана в заголовке пакета. Может использоваться за пределами пакета; +
1 -- если константа описана в теле пакета. Не может использоваться за пределами пакета.

|RDB$CONSTANT_BLR
|BLOB SUB_TYPE 2
|Двоичное представление (BLR) кода выражения для инициализации константы.

|RDB$CONSTANT_SOURCE
|BLOB SUB_TYPE TEXT CHARACTER SET UTF8
|Исходный код выражения для инициализации константы на языке SQL.

|RDB$SCHEMA_NAME
|CHAR(63) CHARACTER SET UTF8
|Имя схемы пакета в котором объявлена константа.

|RDB$DESCRIPTION
|BLOB SUB_TYPE TEXT CHARACTER SET UTF8
|Текст примечания для константы.
...
В таблице `RDB$FIELDS` хранятся определения именованных и сгенерированных системой доменов. В этой таблице хранится информация о типах доменов, столбцов и параметров. Столбцы и параметры могут иметь переопределяющую или более специфичную информацию о типах в своих собственных системных таблицах.

На столбцы `RDB$FIELDS.RDB$SCHEMA_NAME` и `RDB$FIELDS.RDB$FIELD_NAME` ссылаются:

* столбцы `RDB$SCHEMA_NAME` и `RDB$FIELD_NAME` в <<fblangref-systables-field_dimensions,`RDB$FIELD_DIMENSIONS`>>
* столбцы `RDB$FIELD_SOURCE_SCHEMA_NAME` и `RDB$FIELD_SOURCE` в <<fblangref-systables-function_arguments,`RDB$FUNCTION_ARGUMENTS`>>
* столбцы `RDB$FIELD_SOURCE_SCHEMA_NAME` и `RDB$FIELD_SOURCE` в <<fblangref-systables-procedure_parameters,`RDB$PROCEDURE_PARAMETERS`>>
* столбцы `RDB$FIELD_SOURCE_SCHEMA_NAME` и `RDB$FIELD_SOURCE` в <<fblangref-systables-relation_fields,`RDB$RELATION_FIELDS`>>

.Описание столбцов таблицы `RDB$FIELDS`
[cols="<3m,<3m,<6a", frame="all", options="header",stripes="none"]
|===
^| Наименование
^| Тип данных
^| Описание
...
|RDB$FIELD_NAME
|CHAR(63) CHARACTER SET UTF8
|Имя домена, созданного пользователем, или домена, автоматически построенного  Firebird для столбца таблицы. Имя доменов сгенерированных системой начинаться с символов `RDB$`.
...
`DOUBLE PRECISION`, `BIGINT`, `TIMESTAMP`, `TIME WITH TIME ZONE`, `DECFLOAT(16)` и `BLOB` занимают 8 байтов. +
`TIMESTAMP WITH TIME ZONE` занимает 12 байт. +
`INT128` и `DECFLOAT(34)` занимают 16 байт. +
Для `CHAR` и `VARCHAR` указывает максимальное количество байтов, которое может вместится в этот тип (зависит от объявленного размера и кодировки символов).

|RDB$FIELD_SCALE
|SMALLINT
|Задаёт масштаб для доменов типа  `DECIMAL` и `NUMERIC`.

Например, для `NUMERIC(p,s)` значение равно `-1 * s`, где `s` -- количество цифр после запятой.
...
См. также описание столбца `RDB$FIELD_SUB_TYPE`

|RDB$FIELD_SUB_TYPE
|SMALLINT
|Подтип для типа данных:
...
* `4` - `RANGES`
* `5` - `SUMMARY`
* `6` - `FORMAT` -- описание метаданных таблицы
* `7` - `TRANSACTION_DESCRIPTION`
* `8` - `EXTERNAL_FILE_DESCRIPTION`
* `9` - `DEBUG_INFORMATION`
* `>9` - зарезервировано для будущего использования Firebird
* `<0` - пользовательские подтипы BLOB

Для целочисленных типов данных (`SMALLINT`, `INTEGER`, `BIGINT`, `INT128`), и `DOUBLE PRECISION` (в первом диалекте):

[options="compact"]
* `0` - (или `NULL`) без подтипа -- тип данных определяется `RDB$FIELD_TYPE`. +
Если `RDB$FIELD_SCALE` не равен нулю, то обрабатывается как `NUMERIC`.
* `1` - `NUMERIC`
* `2` - `DECIMAL`
...
* `0` - (или `NULL`) без подтипа -- тип данных определяется `RDB$FIELD_TYPE`
* `1` - `BINARY`/`VARBINARY`
* `>1` - не типизированный -- тип данных определяется `RDB$FIELD_TYPE`. +
Внутреннее использование для системных доменов, связанных с кодировкой символов.
...
|RDB$DESCRIPTION
|BLOB SUB_TYPE TEXT CHARACTER SET UTF8
|Описание домена заданное пользователем (текст `COMMENT ON`).
...
[options="compact"]
* `0` - (или `NULL`) домен определён пользователем
* `1` - системный домен
...
|RDB$SEGMENT_LENGTH
|SMALLINT
|Рекомендуемая длина сегмента (в байтах) для `BLOB`. `NULL` для всех остальных типов данных.

Устарело и предназначено в основном для ESQL.
...
|RDB$EXTERNAL_LENGTH
|SMALLINT
|Длина столбца в байтах, если он входит в состав внешней таблицы. `NULL` для обычных таблиц.

|RDB$EXTERNAL_SCALE
|SMALLINT
|Масштаб для целочисленных полей внешней таблицы.

См. также описание столбца `RDB$FIELD_SCALE`.

|RDB$EXTERNAL_TYPE
|SMALLINT
|Тип данных поля, как он представляется во внешней таблице.

См. также описание столбца `RDB$FIELD_TYPE`.
...
|RDB$NULL_FLAG
|SMALLINT
|Указывает, может ли домен принимать значение `NULL`

[options="compact"]
* `0` - (или `NULL`) `NULL` значения допускаются
* `1` - `NULL` не допускается

|RDB$CHARACTER_LENGTH
|SMALLINT
|Максимальное длина в символах для `CHAR` или `VARCHAR`. +
Для `BINARY` или `VARBINARY` -- максимальное количество байт.
...
|RDB$CHARACTER_SET_ID
|SMALLINT
|Идентификатора набора символов для символьного типа или `BLOB SUB_TYPE TEXT`.

|RDB$FIELD_PRECISION
|SMALLINT
|Указывает общее количество цифр для числового типа данных с фиксированной точкой (_p_ в `DECIMAL(p[,s])`, `NUMERIC(p[,s])` и `DECFLOAT(p)`. Для целочисленных типов данных значением является 0. Для всех остальных типов данных -- `NULL`.
...
|RDB$AGGREGATE_FLAG
|SMALLINT
|Флаг агрегатной функции.

1 - функция является агрегатной; +
`NULL` или 0 - функция является скалярной.
...
|RDB$PACKAGE_NAME
|CHAR(63) CHARACTER SET UTF8
|Имя пакета, для индексов упакованных временных таблиц.
...
0 -- не уникальный; +
1 -- уникальный; +
3 -- уникальный не проверенный (not validated).
...
|RDB$PACKAGE_NAME
|CHAR(63) CHARACTER SET UTF8
|Имя пакета, в котором расположена упакованная временная таблица.

|===

.Список столбцов таблицы `COLOR` схемы `PUBLIC`
[example]
====
[source,sql]
----
...
WHERE F.RDB$SCHEMA_NAME = 'PUBLIC'
  AND F.RDB$PACKAGE_NAME IS NULL
  AND F.RDB$RELATION_NAME = 'COLOR'
ORDER BY F.RDB$FIELD_POSITION
----
====

.Список столбцов временной таблицы `PKG_T_PRIV` пакета `TEST_PKG` схемы `PUBLIC`
[example]
====
[source,sql]
----
...
FROM RDB$RELATION_FIELDS F
WHERE F.RDB$SCHEMA_NAME = 'PUBLIC'
  AND F.RDB$PACKAGE_NAME = 'TEST_PKG'
  AND F.RDB$RELATION_NAME = 'PKG_T_PRIV'
ORDER BY F.RDB$FIELD_POSITION
----
====
...
|RDB$PACKAGE_NAME
|CHAR(63) CHARACTER SET UTF8
|Имя пакета в которой расположена таблица (для упакованных временных таблиц).

|RDB$PRIVATE_FLAG
|SMALLINT
|Используется только для упакованных временных таблиц, для остальных имеет значение `NULL`.

0 -- публичная упакованная временная таблица (объявлена в заголовке пакета); +
1 -- частная упакованная временная таблица (объявлена в теле пакета).

|===

.Список таблиц и представлений текущей схемы
[example]
====
[source,sql]
----
...
WHERE R.RDB$SCHEMA_NAME = CURRENT_SCHEMA
  AND R.RDB$PACKAGE_NAME IS NULL
ORDER BY R.RDB$RELATION_NAME
----
====

.Список таблиц и представлений доступные для текущего пути поиска
[example]
====
[source,sql]
----
SELECT
  ...
  R.RDB$SYSTEM_FLAG
FROM RDB$RELATIONS R
WHERE R.RDB$SCHEMA_NAME IN (
    SELECT N.NAME
    FROM RDB$SQL.PARSE_UNQUALIFIED_NAMES(RDB$GET_CONTEXT('SYSTEM', 'SEARCH_PATH')) N
  )
  AND R.RDB$PACKAGE_NAME IS NULL
ORDER BY R.RDB$SCHEMA_NAME, R.RDB$RELATION_NAME
----
====

.Список временных таблиц пакета `TEST_PKG` принадлежащего схеме `PUBLIC`
[example]
====
[source,sql]
----
SELECT
  ...
  R.RDB$PRIVATE_FLAG
FROM RDB$RELATIONS R
WHERE R.RDB$SCHEMA_NAME = 'PUBLIC'
  AND R.RDB$PACKAGE_NAME = 'TEST_PKG'
ORDER BY R.RDB$RELATION_NAME
----
====
