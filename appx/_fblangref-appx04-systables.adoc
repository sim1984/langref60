...
* столбцы `RDB$SCHEMA_NAME` и `RDB$FIELD_NAME` в таблице <<fblangref-systables-field_dimensions,`RDB$FIELD_DIMENSIONS`>>
* столбцы `RDB$FIELD_SOURCE_SCHEMA_NAME` и `RDB$FIELD_SOURCE` в таблице <<fblangref-systables-function_arguments,`RDB$FUNCTION_ARGUMENTS`>>
* столбцы `RDB$FIELD_SOURCE_SCHEMA_NAME` и `RDB$FIELD_SOURCE` в таблице <<fblangref-systables-procedure_parameters,`RDB$PROCEDURE_PARAMETERS`>>
* столбцы `RDB$FIELD_SOURCE_SCHEMA_NAME` и `RDB$FIELD_SOURCE` в таблице <<fblangref-systables-relation_fields,`RDB$RELATION_FIELDS`>>
...
`4`:: `RANGES`
`5`:: `SUMMARY`
`6`:: `FORMAT` -- закодированное описание метаданных таблицы
`7`:: `TRANSACTION_DESCRIPTION`
`8`:: `EXTERNAL_FILE_DESCRIPTION`
`9`:: `DEBUG_INFORMATION`
`>9`:: зарезервировано для внутреннего или будущего использования Firebird
