...
<<fblangref-montables-local_tables>>::
Сведения об активных локальных временных таблицах (LTT) для всех соединений.

<<fblangref-montables-local_table_columns>>::
Предоставляет информацию о столбцах активных локальных временных таблицах (LTT) для всех соединений.
...
Предоставляет информацию об активных локальных временных таблицах (LTT) для всех соединений.

.Описание столбцов таблицы `MON$LOCAL_TEMPORARY_TABLES`
[cols="<4m,<3m,<5", frame="all", options="header",stripes="none"]
|===
^| Наименование столбца
^| Тип данных
...
|MON$TABLE_ID
|INTEGER
|Внутренний идентификатор таблицы (уникальный внутри соединения).
...
|Тип таблицы: (`PRESERVE ROWS` или `DELETE ROWS`).

|===

.Получение списка локальных таблиц созданных в своём соединении
[example]
====
[source,sql]
----
SELECT LTT.MON$SCHEMA_NAME, LTT.MON$TABLE_NAME, LTT.MON$TABLE_TYPE
FROM
   SYSTEM.MON$LOCAL_TEMPORARY_TABLES LTT
WHERE LTT.MON$ATTACHMENT_ID = CURRENT_CONNECTION
ORDER BY LTT.MON$SCHEMA_NAME, LTT.MON$TABLE_NAME
----
====
...
.Описание столбцов таблицы `MON$LOCAL_TEMPORARY_TABLE_COLUMNS`
[cols="<4m,<3m,<5", frame="all", options="header",stripes="none"]
|===
^| Наименование столбца
^| Тип данных
...
|MON$CHAR_LENGTH
|SMALLINT
|Максимальная длина в символах для строковых типов.
...
|MON$NOT_NULL
|SMALLINT
|Возможность содержать значение `NULL` (1 для `NOT NULL` полей, 0 - для nullable).
...
|MON$COLLATION_ID
|SMALLINT
|Идентификатор сопоставления символов.

|===

.Получение списка столбцов для локальной таблицы с заданным именем, созданной в своём соединении
[example]
====
[source,sql]
----
SELECT *
FROM
   SYSTEM.MON$LOCAL_TEMPORARY_TABLE_COLUMNS C
WHERE C.MON$ATTACHMENT_ID = CURRENT_CONNECTION
  AND C.MON$TABLE_NAME = 'TEMP_DATA'
  AND C.MON$SCHEMA_NAME = 'PUBLIC'
ORDER BY C.MON$FIELD_POSITION
----
====
...
`PERSISTENT` - обычная постоянная таблица; +
`GLOBAL TEMPORARY` - глобальная временная таблица; +
`LOCAL TEMPORARY` - локальная временная таблица.

Упакованные временные таблицы имеют тип - `GLOBAL TEMPORARY`.
