...
<object> ::=
  ...
  | <schema-less-type> _objectname_
  | USER _username_ [USING PLUGIN _plugin_name_]
  | TABLE [<relation-location>] _relationname_
  | COLUMN [<relation-location>] _relationname_ <period> _fieldname_
  | {PROCEDURE | [EXTERNAL] FUNCTION}
    [<routine-location>] _routinename_
  | [PROCEDURE | FUNCTION] PARAMETER
    [<routine-location>] _routinename_ <period> _paramname_
  | CONSTANT [ _package-schema_ <period> ] _package-name_ <period> _constant-name_
...
<relation-location> ::=
    _relation-schema_
  | [ _relation-schema_ <period> ] _package-name_
  | _relation-name_%PACKAGE
  | _relation-schema_%SCHEMA
...
При добавлении комментария для пользователя вы можете уточнить, в каком плагине управления пользователями он находится, с помощью необязательного предложения `USING PLUGIN`. Если это предложение отсутствует, то предполагается, что пользователь создан в плагине управления по умолчанию, то есть первого плагина, указанного в параметре UserManager в файле `firebird.conf` или `databases.conf`.
...
[source,sql]
----
-- добавляется комментарий к столбцу первой найденной в путях поиска таблицы METALS
...
.Добавление комментария для пакета, его процедур и функций, их параметров, а также констант
[example]
====
[source,sql]
----
...
COMMENT ON CONSTANT APP_VAR.MIN_DATE
IS 'Минимально допустимая дата периода';

COMMENT ON CONSTANT APP_VAR.MAX_DATE
IS 'Максимально допустимая дата периода';
...
.Добавление комментария для пакета из схемы APP, его процедур и функций, их параметров
[example]
====
[source,sql]
----
...
COMMENT ON CONSTANT APP.APP_VAR.MIN_DATE
IS 'Минимально допустимая дата периода';

COMMENT ON CONSTANT APP.APP_VAR.MAX_DATE
IS 'Максимально допустимая дата периода';
...
.Добавление комментария для временных таблиц пакета и его столбцов
[example]
====
[source,sql]
----
