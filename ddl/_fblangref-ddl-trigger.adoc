...
Если используется предложение `IF NOT EXISTS`, то оператор `CREATE TRIGGER` попытается создать новый триггер без выдачи ошибки, если он уже существует.
DDL триггеры не будут запускаться, если создаваемый триггер уже существует.
...
Если используется предложение `IF EXISTS`, то оператор `DROP TRIGGER` попытается удалить триггер без выдачи ошибки, если его не существует.
DDL триггеры не будут запускаться, если удаляемый триггер не существует.
