...
Имя схемы для DML триггера (явно указанное или текущая схема) должно быть тем же самым, что и имя схемы для таблицы (представления) для которой создаётся этот триггер, в противном случае будет выдана соответствующая ошибка.
