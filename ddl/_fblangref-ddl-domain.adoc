...
----
CREATE DOMAIN [IF NOT EXISTS] _name_ [AS] <datatype>
...
[listing,subs=+quotes]
----
DROP DOMAIN [IF EXISTS] _domain_name_
----
...
Если используется предложение `IF EXISTS`, то оператор `DROP DOMAIN` попытается удалить домен без выдачи ошибки, если его не существует.
DDL триггеры не будут запускаться, если домен не существует.
