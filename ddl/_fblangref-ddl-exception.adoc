...
[listing,subs="+quotes"]
----
CREATE EXCEPTION [IF NOT EXISTS] [ _exc-schema_ <period> ] _exception_name_ '<message>'
...
|exc-schema
|Имя схемы исключения. Если не указано, то исключение создаётся в текущей схеме.

|exception_name
|Имя исключения. Максимальная длина 63 символа.
...
Нумерация начинается с 1. Максимальный номер слота равен 9.
|===
...
[WARNING]
====
Если в тексте сообщения встретится номер слота параметра больше 9, то второй и последующий символы будут восприняты как литералы. Например, @10 будет воспринято как @1, после которого следует литерал 0.
====
...
----
ALTER EXCEPTION [ _exc-schema_ <period> ] _exception_name_ '<message>'
...
[listing,subs="+quotes,macros"]
----
CREATE OR ALTER EXCEPTION [ _exc-schema_ <period> ] _exception_name_ '<message>'
...
[listing,subs=+quotes]
----
DROP EXCEPTION [IF EXISTS] [ _exc-schema_ <period> ] _exception_name_
----
...
При наличии зависимостей для существующего исключения удаление не будет выполнено.
...
[listing,subs="+quotes,macros"]
----
RECREATE EXCEPTION [ _exc-schema_ <period> ] _exception_name_ '<message>'
