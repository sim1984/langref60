...
* Если исходный столбец основан на именованном домене, новый столбец ссылается на этот домен напрямую, как если бы он был объявлен как `<column name> <domain name>`;
...
Для столбцов типов `CHAR` и `VARCHAR` не используется кодировка символов базы данных по умолчанию. Кодировка символов и параметры сортировки нового столбца копируются из соответствующего выражения запроса.
