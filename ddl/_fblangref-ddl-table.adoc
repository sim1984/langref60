...
Таблицы бывают следующий типов:
...
В этой форме новые столбцы таблицы формируются на основе списка столбцов запроса, возвращающего набор данных. Если указан список имен столбцов, он должен содержать то же количество столбцов, что и результат запроса.

Если список имен столбцов не указан, имена столбцов берутся из имен выходных данных запроса. Для всех безымянных выражений в списке столбцов запроса должны быть явно указаны псевдонимы.
...
* Если исходный столбец основан на именованном домене, то новый столбец ссылается на этот домен напрямую, как если бы он был объявлен как `<column name> <domain name>`. Однако, если столбец находится в контексте внешнего соединения,
где он допускает значение `NULL`, и домен объявлен как `NOT NULL`, то новый столбец вместо этого использует копию базового типа домена (с сохранением точности, масштаба, набора символов, сопоставления и т. д.) без
...
.Создание таблицы на основе результатов запроса с переименование столбцов
[example]
====
