...
* Глобальные временные таблицы (`GLOBAL TEMPORARY TABLE` или GTT). Описаны в <<fblangref_ddl_gtt>>;
* Локальные временные таблицы (`LOCAL TEMPORARY TABLE` или LTT). Описаны в <<fblangref_ddl_ltt>>;
* Внешние (EXTERNAL) таблицы. Описаны в <<fblangref_ddl_table_ext>>;
* Виртуальные (таблицы мониторинга `MON$` и виртуальные таблицы безопасности `SEC$`). Пользователи не могут создавать свои виртуальные таблицы.

В данном разделе рассматриваются вопросы создания, модификации и удаления постоянных таблиц базы данных.
...
----
CREATE [{GLOBAL | LOCAL} TEMPORARY] TABLE [IF NOT EXISTS] [ _tbl-schema_ <period> ] _tablename_
...
[[fblangref-ddl-tbl-create-as-query-syntax]]
.Синтаксис создания таблицы на основе запроса
[listing,subs="+quotes,macros,attributes"]
----
CREATE [{GLOBAL | LOCAL} TEMPORARY] TABLE [IF NOT EXISTS] [ _tbl-schema_ <period> ] _tablename_
...
|query expression
|SQL оператор возвращающий набор данных.
...
Таблица должна содержать, по меньшей мере, один столбец и произвольное количество ограничений таблицы.
...
[IMPORTANT]
====
Следует обратить внимание на то, что если в определении домена было указано `NOT NULL`, на уровне столбца невозможно определить допустимость использования в нем значения `NULL`. Если вы хотите, чтобы на основе домена можно было определять столбцы, допускающие псевдозначение `NULL` и не допускающие его, то хорошей практикой является создание домена, допускающего `NULL`, и указание ограничения `NOT NULL` у столбцов таблицы там, где это необходимо.
====
...
Чтобы использовать сгенерированное по умолчанию значение, необходимо либо указать ключевое слово `DEFAULT` при вставке в столбец идентификации, либо просто не упоминать столбец идентификации в списке столбцов для вставки. В противном случае будет использовано указанное вами значение.
...
Необязательное предложение `START WITH`(((CREATE TABLE, IDENTITY, START WITH))) позволяет указать начальное значение, отличное от нуля. Предложение `INCREMENT [BY]`(((CREATE TABLE, IDENTITY, INCREMENT BY))) устанавливает значение приращения. Значение приращения должно быть отлично от 0. По умолчанию значение приращения равно 1.
...
Для любого типа данных, кроме `BLOB`, можно указать размерность массива, если столбец должен быть массивом. Размерность массива указывается в квадратных скобках. Чтобы не перепутать их с символами, означающими необязательные элементы, они выделены жирным шрифтом.

При указании размерности массива указываются два числа через двоеточие. Первое число означает начальный номер элемента массива, второе -- конечный. Если указано только одно число, то оно означает последний номер в элементе массива, а первым номером считается 1. Для многомерного массива размерности массива перечисляются через запятую.
...
Синтаксис определения внешнего ключа на уровне таблицы несколько отличается. После определения всех столбцов с их ограничениями уровня столбца вы можете определить именованное ограничение внешнего ключа уровня таблицы, используя ключевые слова `FOREIGN KEY` и имена столбцов, для которых оно применяется:
...
[[fblangref-ddl-table-create-as-query]]
=== Создание таблицы на основе запроса

Оператор `CREATE TABLE` имеет специальную форму (см. <<fblangref-ddl-tbl-create-as-query-syntax>>) для создания таблицы по результатам запроса.

В этой форме новые столбцы таблицы формируются на основе списка столбцов запроса, возвращающего набор данных. Если указан список имён столбцов, он должен содержать то же количество столбцов, что и результат запроса.

Если список имён столбцов не указан, имена столбцов берутся из имён выходных данных запроса. Для всех безымянных выражений в списке столбцов запроса должны быть явно указаны псевдонимы.

Данная форма оператора `CREATE TABLE` имеет необязательную опцию `WITH [NO] DATA`:

* `WITH DATA` используется по умолчанию и вставляет результат запроса в новую таблицу, созданную в той же транзакции;
* `WITH NO DATA` создает только определение таблицы.

Для глобальных и локальных временных таблиц действуют обычные правила определения времени жизни данных во временных таблицах. Создание внешних таблиц по результатам запроса не поддерживается.

Согласно SQL стандарту, запрос, на основе которого создаётся таблица, должен быть заключён в круглые скобки. Во многих других популярных СУБД заключение запроса в круглые скобки не является обязательным, что является отступлением от стандарта. Firebird следует тому же принципу для максимальной совместимости с другими СУБД.

==== Типы столбцов

Когда элемент списка `SELECT` является прямой ссылкой на столбец исходной таблицы или представления, новый столбец копирует точный тип данных исходного столбца, а не тот, который определяется только на основе результата выполнения запроса (что привело бы к потере информации, например, объявленной точности `NUMERIC`/`DECIMAL`).

* Если исходный столбец основан на именованном домене, то новый столбец ссылается на этот домен напрямую, как если бы он был объявлен как `<column name> <domain name>`. Однако, если столбец находится в контексте внешнего соединения, где он допускает значение `NULL`, и домен объявлен как `NOT NULL`, то новый столбец вместо этого использует копию базового типа домена (с сохранением точности, масштаба, набора символов, сопоставления и т. д.) без ссылки на исходный домен.
* Если исходный столбец основан на автоматически сгенерированном (неявном) домене, копируется его точный тип (точность/масштаб, набор символов, сопоставление и т. д.).

Для любого другого элемента выпадающего списка (например, выражения, литерала или агрегатной функции) тип данных нового столбца определяется на основе результата запроса.

Для столбцов `CHAR` и `VARCHAR`, не охваченных правилом прямой ссылки на столбец, описанным выше, кодировка символов базы данных по умолчанию не используется. Кодировка символов и параметры сортировки нового столбца копируются из соответствующего выражения запроса.

Строковые литералы используют кодировку символов соединения, поэтому столбцы, созданные из строковых литералов, наследуют кодировку символов соединения.
...
CREATE TABLE objects (
  id INTEGER GENERATED BY DEFAULT AS IDENTITY (START WITH 10 INCREMENT BY 2) PRIMARY KEY,
  name VARCHAR(15)
);
...
То же самое, но вычисляемые столбцы вычисляются с правами определяющего пользователя (владельца таблицы). Кроме того, триггеры наследуют привилегии выполнения таблицы, если они не переопределены у самих триггеров.
====

.Создание таблицы на основе результатов запроса
[example]
====
В данном примере данные из запроса копируются в таблицу.
...
.Создание таблицы на основе результатов запроса с переименованием столбцов
[example]
====
В данном примере данные из запроса не копируются в таблицу.

[source,sql]
----
CREATE TABLE employee_names (id, full_name)
AS (
  SELECT emp_no, first_name || ' ' || last_name
  FROM employee
) WITH NO DATA;
...
.См. также:
<<fblangref-ddl-table-alter>>, <<fblangref-ddl-table-alter>>, <<fblangref-ddl-domain-create>>.
...
Предложение `ADD [COLUMN]` позволяет добавить новый столбец. Синтаксис определения столбца таблицы полностью совпадает с синтаксисом, описанным в операторе <<fblangref-ddl-table-create,CREATE TABLE>>.
...
Предложение `ADD [CONSTRAINT]` позволяет добавить новое ограничение таблицы. Синтаксис описания ограничения таблицы полностью совпадает с синтаксисом, описанным в операторе <<fblangref-ddl-table-create,CREATE TABLE>>.
...
-- Схема явно указана для обоих таблиц
ALTER TABLE APP.JOB
ADD FOREIGN KEY (JOB_COUNTRY)
...
Если для строкового столбца необходимо изменить порядок сортировки, то это можно сделать через изменение типа. В этом случае вам придётся полностью указать тип данных, набор символов и порядок сортировки.
...
Если указано только предложение `RESTART`, то происходит сброс значения генератора в ноль. Необязательное предложение `WITH` позволяет указать для нового значения внутреннего генератора значение, отличное от нуля.
...
Предложение `DROP IDENTITY` удаляет связанную со столбцом идентификации системную последовательность и преобразует его в обычный столбец.
...
По умолчанию вычисляемые столбцы вычисляются с привилегиями, установленными в качестве атрибута схемы, которой принадлежит таблица. Если схеме не установлен атрибут `SQL SECURITY`, то для этой схемы привилегии выполнения наследуются от соответствующего атрибута, установленного для базы данных. Если для базы данных атрибут не установлен, то будут использованы привилегии вызывающего пользователя. Кроме того, триггеры наследуют привилегии выполнения у таблицы, если они не переопределены у самих триггеров.

.Изменение привилегий выполнения для первой найденной таблицы с именем COUNTRY в одной из схем в пути поиска
[example]
====
[source,sql]
----
...
.Изменение привилегий выполнения для таблицы с именем COUNTRY в схеме MANAGEMENT
[example]
====
[source,sql]
----
...
.См. также:
<<fblangref-ddl-table-create>>, <<fblangref-ddl-table-recreate>>.
...
.Удаление первой найденной таблицы с именем COUNTRY в одной из схем в пути поиска
[example]
====
[source,sql]
----
...
.Удаление таблицы с именем COUNTRY из схемы MANAGEMENT
[example]
====
[source,sql]
----
...
[[fblangref-ddl-tbl-recreate-syntax]]
.Синтаксис пересоздания таблицы
[listing,subs="+quotes"]
----
RECREATE [{GLOBAL | LOCAL} TEMPORARY] TABLE [ _tbl-schema_ <period> ] _tablename_
  [EXTERNAL [FILE] '_filespec_']
  (<col_def> [, <col_def> | <tconstraint> ...])
  [ON COMMIT {DELETE | PRESERVE} ROWS]
  [SQL SECURITY {DEFINER | INVOKER}]
...
[[fblangref-ddl-tbl-recreate-as-query-syntax]]
.Синтаксис пересоздания таблицы на основе запроса
[listing,subs="+quotes,macros,attributes"]
----
RECREATE [{GLOBAL | LOCAL} TEMPORARY] TABLE [ _tbl-schema_ <period> ] _tablename_
  [ (<column name> [, <column name> ...]) ]
...
.Пересоздание таблицы на основе запроса
[example]
====
[source,sql]
----
RECREATE TABLE employee_copy
AS
SELECT emp_no, first_name, last_name
FROM employee
WHERE emp_no > 10;
...
.См. также:
<<fblangref-ddl-table-create>>, <<fblangref-ddl-table-drop>>.
