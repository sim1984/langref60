...
Столбцы создаваемой таблицы копируют тип данных и набор символов выражений указанных для столбцов в запросе. Все столбцы таблицы имеют возможностью принимать `NULL` значения.
...
Для глобальных и локальных временных таблиц действуют обычные правила определения времени жизни данных во временных таблицах. Создание внешних таблиц не поддерживается по результатам запроса не поддерживается.

[[fblangref-ddl-table-create-as-query-charset]]
==== Наборы символов

Для столбцов типов `CHAR` и `VARCHAR` не используется кодировка символов базе данных по умолчанию. Кодировка символов и параметры сортировки нового столбца копируются из соответствующего выражения запроса.
