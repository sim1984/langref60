...
|misc-info
|Определяемая пользователем информация для передачи в процедуру внешнего модуля.
...
В необязательной секции `<declarations>` описаны локальные переменные процедуры, подпрограммы, именованные курсоры и локальные временные таблицы. В отношении спецификации типа данных локальные переменные подчиняются тем же правилам, что и входные и выходные параметры процедуры. Подробности вы можете посмотреть в главе "`Процедурный язык PSQL`" в разделах
...
-- Домен D_BREEDNAME, таблица BREED должны находиться в той же схеме, что и процедура.
...
-- Таблица BREED должна находиться в схеме MAIN, поскольку она не квалифицирована.
...
После выполнения оператора `ALTER PROCEDURE` существующие привилегии и зависимости сохраняются.
...
Оператор `ALTER PROCEDURE` позволяет изменить привилегии выполнения процедуры без повторного указания входных и выходных параметров и тела процедуры. Для установки привилегий выполнения используется предложение `SQL SECURITY` с одной из опций: `INVOKER` или `DEFINER`. Если необходимо удалить привилегии выполнения процедуры, укажите предложение `DROP SQL SECURITY`.
...
[source,sql]
----
-- Устанавливает процедуре, что она будет выполняться
-- с привилегиями вызывающего пользователя.
...
Если от хранимой процедуры существуют зависимости, то при попытке удаления такой процедуры будет выдана соответствующая ошибка.
