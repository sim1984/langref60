...
Имя хранимой функции должно быть уникальным среди имён всех хранимых скалярных, агрегатных и внешних (Legacy UDF) функций внутри заданной (текущей) схемы.
...
`CREATE FUNCTION` является составным оператором, состоящим из заголовка и тела. Заголовок определяет имя хранимой функции, объявляет входные параметры и тип возвращаемого значения.
...
Входные параметры заключаются в скобки после имени хранимой функции. Они передаются в функцию по значению, то есть любые изменения входных параметров внутри функции никак не повлияют на значения этих параметров в вызывающей программе.
...
В необязательной секции `<declarations>` описаны локальные переменные функции, именованные курсоры, подпрограммы (подпроцедуры и подфункции) и локальные временные таблицы. Локальные переменные подчиняются тем же правилам, что и входные параметры функции в отношении спецификации типа данных. Подробности вы можете посмотреть в главе "`Процедурный язык PSQL`" в разделах <<fblangref-psql-statements-declare-var>>,
...
[WARNING]
====
Не следует путать внешние функции, объявленные как `DECLARE EXTERNAL FUNCTION`, так же известные как UDF, с функциями, расположенными во внешних модулях и объявленными как `CREATE FUNCTION ... EXTERNAL NAME`, называемыми UDR (User Defined Routine). Первые являются унаследованными (Legacy) из предыдущих версий Firebird. Их возможности существенно уступают возможностям нового типа внешних функций. В Firebird 4.0 UDF объявлены устаревшими.
====
...
----
-- функция создаётся в текущей схеме (первой валидной схеме в путях поиска SEARCH_PATH)
-- Таблицу RDB$TYPES можно не квалифицировать, поскольку путь поиска
...
.Создание хранимой функции, содержащей подфункцию
[example]
====
...
* тело хранимой функции;
* флаг, является ли функция детерминированной или недетерминированной;
* устанавливать или удалять привилегии выполнения (SQL SECURITY).

Невозможно преобразовать скалярную функцию в агрегатную и наоборот.

Для внешних функций (UDR) вы можете изменить точку входа и имя движка. Внешние функции, объявленные как `DECLARE EXTERNAL FUNCTION`, так же известные как UDF, невозможно преобразовать в PSQL функции и наоборот.

После выполнения оператора `ALTER FUNCTION` существующие привилегии и зависимости сохраняются.
...
[WARNING]
====
Если у вас уже есть внешняя функция в Legacy стиле (`DECLARE EXTERNAL FUNCTION`), то оператор `ALTER FUNCTION` изменит её на обычную функцию без всяких предупреждений. Это было сделано умышленно для облегчения миграции на новый стиль написания внешних функций, известных как UDR.
====

[[fblangref-ddl-function-alter_deterministic]]
=== Признак детерминированной функции

(((ALTER FUNCTION, DETERMINISTIC)))
Оператор `ALTER FUNCTION` позволяет изменить признак, является ли функция детерминированной или недетерминированной, без повторного указания входных параметров, выходного результата и тела функции. Для пометки функции как детерминированной укажите ключевое слово `DETERMINISTIC`. Если необходимо сделать функцию недетерминированной -- укажите `NOT DETERMINISTIC`.

[[fblangref-ddl-function-alter_sql_security]]
=== Привилегии выполнения

(((ALTER FUNCTION, SQL SECURITY)))
Оператор `ALTER FUNCTION` позволяет изменить привилегии выполнения функции без повторного указания входных параметров, выходного результата и тела функции. Для установки привилегий выполнения используется предложение `SQL SECURITY` с одной из опций: `INVOKER` или `DEFINER`. Если необходимо удалить привилегии выполнения функции, укажите предложение `DROP SQL SECURITY`.
...
.Изменение только признака детерминированности функции
[example]
====
[source,sql]
----
-- Помечает функцию как детерминированную
ALTER FUNCTION ADD_INT DETERMINISTIC;
-- Помечает функцию как недетерминированную
ALTER FUNCTION ADD_INT NOT DETERMINISTIC;
----
====

.Изменение только привилегий выполнения функции
[example]
====
[source,sql]
----
-- Устанавливает для функции, что она будет выполняться
-- с привилегиями вызывающего пользователя.
ALTER FUNCTION ADD_INT SQL SECURITY INVOKER;
-- Устанавливает для функции, что она будет выполняться
-- с привилегиями определяющего пользователя (владельца функции).
ALTER FUNCTION ADD_INT SQL SECURITY DEFINER;
-- Удаляет привилегии выполнения для функции.
-- После выполнения этого оператора привилегии выполнения будут унаследованы
-- от схемы, в которой создана эта функция.
...
[WARNING]
====
Если у вас уже есть внешняя функция в Legacy стиле (`DECLARE EXTERNAL FUNCTION`), то оператор `CREATE OR ALTER FUNCTION` изменит её на обычную функцию без всяких предупреждений. Это было сделано умышленно для облегчения миграции на новый стиль написания внешних функций, известных как UDR.
====
...
.Назначение
Удаление хранимой скалярной или агрегатной функции.
(((DROP FUNCTION)))
...
[WARNING]
====
Скалярные и пользовательские агрегатные функции имеют одно и то же пространство имён. Это обозначает, что `RECREATE  FUNCTION` может удалить агрегатную функцию и создать скалярную функцию с тем же именем.
====
