...
* Уничтожение экземпляра GTT в конце своего жизненного цикла не вызывает срабатывания триггеров `{BEFORE | AFTER} DELETE`.
...
(((CREATE GLOBAL TEMPORARY TABLE)))
Глобальные временные таблицы создаются с помощью оператора `CREATE GLOBAL TEMPORARY TABLE`. Синтаксис этого оператора представлен ниже:

[[fblangref-ddl-gtt-create-syntax]]
...
[[fblangref-ddl-gtt-create-as-query-syntax]]
.Синтаксис создания глобальной временной таблицы по результатам запроса
[listing,subs=+quotes]
----
CREATE GLOBAL TEMPORARY TABLE [IF NOT EXISTS] [ _tbl-schema_ <period> ] _tablename_
...
=== Создание глобальных временных таблиц на основе результатов запроса

Оператор `CREATE GLOBAL TEMPORARY TABLE` имеет специальную форму (см. <<fblangref-ddl-gtt-create-as-query-syntax>>) для создания таблицы по результатам запроса.

Данная форма оператора `CREATE GLOBAL TEMPORARY TABLE` имеет необязательную опцию `WITH [NO] DATA`:
...
Для глобальных временных таблиц действуют обычные правила определения времени жизни данных во временных таблицах.

См. также <<fblangref-ddl-table-create-as-query>>.

==== Поведение в ISQL

В режиме AUTODDL (по умолчанию) ISQL использует отдельные транзакции для операторов DDL и DML. При выполнении команды `CREATE TABLE ... AS <query>` с параметром `WITH DATA` создание таблицы и заполнение данными происходят в транзакции DDL.

Для обычных таблиц вставленные строки не видны транзакции DML до тех пор, пока транзакция DDL не будет зафиксирована. Для временных таблиц `ON COMMIT DELETE ROWS` такое поведение еще более удивительно, поскольку строки относятся к транзакции DDL и, следовательно, вообще не видны транзакции DML. Для таблиц `ON COMMIT PRESERVE ROWS` поведение похоже на поведение `isql` с обычными таблицами.
...
Таблица существует, но строки не возвращаются, поскольку данные были вставлены в транзакции DDL и не видны транзакции DML.

Чтобы избежать такого поведения, отключите AUTODDL перед выполнением оператора:
...
Когда функция AUTODDL отключена, создание таблицы и заполнение данных происходят в текущей транзакции, благодаря чему вставленные строки становятся сразу видимыми.
...
[source,sql]
----
CREATE GLOBAL TEMPORARY TABLE session_report
AS
...
.Синтаксис пересоздания глобальной временной таблицы
[listing,subs="+quotes"]
----
...
.Синтаксис пересоздания глобальной временной таблицы на основании результатов запроса
[listing,subs="+quotes"]
----
RECREATE GLOBAL TEMPORARY TABLE [ _tbl-schema_ <period> ] _tablename_
  [ (<column name> [, <column name> ...]) ]
...
.Пересоздание глобальной временной таблицы
[example]
====
[source,sql]
----
...
.Пересоздание глобальной временной таблицы на основе результатов запроса
[example]
====
[source,sql]
----
RECREATE GLOBAL TEMPORARY TABLE session_report
AS
SELECT emp_no, salary
FROM employee
WITH DATA
