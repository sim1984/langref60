...
|extbody
|Тело внешней процедуры или функции.
...
Все процедуры и функции, объявленные в заголовке пакета, должны быть реализованы в теле пакета. Кроме того, должны быть реализованы и все процедуры и функции, объявленные в теле пакета. Процедуры и функции, определённые в теле пакета, но не объявленные в заголовке пакета, не видны вне тела пакета.

Константы и временные таблицы, объявленные в заголовке пакета, не могут быть повторно объявлены в теле пакета.

Имена процедур, функций, констант и временных таблиц, объявленные в теле пакета, должны быть уникальны среди имён процедур, функций, констант и временных таблиц, объявленных в заголовке и теле пакета.

.Правила:

* В теле пакета должны быть реализованы все подпрограммы с той же сигнатурой, что и объявленные в заголовке и в начале тела пакета.
...
Если указано только имя пакета, то его поиск производится в путях поиска (`SEARCH_PATH`). Будет удалено первое найденное тело пакета с заданным именем среди схем, перечисленных в путях поиска.
...
.Назначение
Создание нового или пересоздание существующего тела пакета.
(((RECREATE PACKAGE BODY)))
