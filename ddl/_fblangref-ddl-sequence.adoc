...
Если используется предложение `IF EXISTS`, то оператор `DROP SEQUENCE` попытается удалить последовательность (генератор) без выдачи ошибки, если её не существует.
DDL триггеры не будут запускаться, если последовательность (генератор) не существует.
