...
* Владелец схемы, в которой расположена последовательность;
* Пользователи с привилегией `ALTER ANY SEQUENCE` (`ALTER ANY GENERATOR`) для схемы, в которой расположена последовательность.
...
При наличии зависимостей для существующей последовательности (генератора) удаление не будет выполнено.
...
* Владелец последовательности (генератора);
* Владелец схемы, в которой расположена последовательность;
* Пользователи с привилегией `DROP ANY SEQUENCE` (`DROP ANY GENERATOR`) для схемы, в которой расположена последовательность.
